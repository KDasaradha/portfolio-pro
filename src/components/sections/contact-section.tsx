...
import { PhoneCall, Mail, Linkedin, Github, Twitter, FileText, Download, Send, ArrowUpRight, Link2 } from 'lucide-react';
...
const contactDetails = [
  { icon: Mail, text: 'kdasaradha525@gmail.com', href: 'mailto:kdasaradha525@gmail.com', label: 'Send an Email', primary: true, highlight: true }, // Added highlight
...
export default function ContactSection() {
  ...
  const formRef = useRef<HTMLFormElement>(null); // Ref for potential form animation
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (cards.length > 0) {
        gsap.from(cards, {
          autoAlpha: 0,
          y: 90, // Increased offset
          scale: 0.92, // Subtle scale effect
          duration: 1.1, // Smoother duration
          ease: 'power3.out',
      ...
      // Optional: Animate form elements if a form is added
      if (formRef.current) {
  ...
  return (
    <section ref={sectionRef} id="contact" className="bg-gradient-to-b from-secondary/10 via-background to-secondary/10 py-32 md:py-40 relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.03] pattern-circuit-board pattern-accent pattern-bg-transparent pattern-size-12 z-0"></div> {/* Changed pattern */}
      
      {/* Enhanced Background Blobs for Contact Section */}
      ...
      <div className="container mx-auto px-4 z-10 relative">
        <h2
          ref={headerRef}
          className="text-4xl md:text-5xl lg:text-6xl font-bold mb-20 md:mb-24 text-center gradient-text tracking-tight" // Added tracking-tight
        >
          Let's Connect & Build Together
        </h2>

        <div ref={cardsRef} className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-stretch"> {/* Added items-stretch */}
          {/* Contact Information Card */}
          <div ref={contactCard1Ref} className="will-change-transform">
            <Card className="h-full shadow-xl hover:shadow-2xl transition-all duration-400 border-t-4 border-accent bg-card/90 backdrop-blur-lg flex flex-col rounded-xl group hover:-translate-y-2.5"> {/* Increased hover effect */}
              <CardHeader className="p-7 md:p-9">
                <CardTitle className="text-2xl md:text-3xl font-semibold text-primary flex items-center gap-3.5">
                  <Send className="h-7 w-7 text-accent transition-transform duration-500 group-hover:rotate-[20deg] group-hover:scale-110" /> {/* Enhanced icon animation */}
                  Get In Touch
                </CardTitle>
              ...
              <CardContent className="flex-grow p-7 md:p-9 pt-0">
                <ul className="space-y-9"> {/* Increased spacing */}
                  {contactDetails.map((detail, index) => (
                    <li key={index} className="flex items-center space-x-5 group/item relative"> {/* Added relative for potential pseudo-elements */}
                      <detail.icon className={cn(
                        "h-8 w-8 flex-shrink-0 transition-all duration-300 group-hover/item:scale-110 group-hover/item:-rotate-[10deg]", // Smoother rotate
                        detail.primary ? 'text-accent' : 'text-muted-foreground group-hover/item:text-accent'
                      )} strokeWidth={1.5} />
                      <Link
                        href={detail.href}
                        className={cn(
                          "text-lg md:text-xl font-medium transition-colors duration-300 group-hover/item:underline underline-offset-4 decoration-accent/70", // Added decoration color
                      ...
                      {detail.primary && <ArrowUpRight className="h-5 w-5 text-accent opacity-0 group-hover/item:opacity-100 transition-all duration-300 group-hover/item:translate-x-1 group-hover/item:-translate-y-1 ml-1.5" />} {/* Enhanced arrow animation */}
                    </li>
                  ))}
                </ul>
              </CardContent>
              <CardFooter className="p-7 md:p-9 border-t mt-auto bg-muted/30 rounded-b-xl"> {/* Changed background */}
          ...
          <div ref={contactCard2Ref} className="will-change-transform">
            <Card className="h-full shadow-xl hover:shadow-2xl transition-all duration-400 border-t-4 border-primary bg-card/90 backdrop-blur-lg flex flex-col rounded-xl group hover:-translate-y-2.5"> {/* Increased hover effect */}
              <CardHeader className="p-7 md:p-9">
                <CardTitle className="text-2xl md:text-3xl font-semibold text-primary flex items-center gap-3.5">
                  <Link2 className="h-7 w-7 text-primary/90 transition-transform duration-500 group-hover:rotate-[10deg] group-hover:scale-110" /> {/* Changed icon, enhanced animation */}
                  Connect & Download
                </CardTitle>
                <CardDescription className="text-base md:text-lg mt-3 text-muted-foreground leading-relaxed">
              ...
              <CardContent className="flex-grow p-7 md:p-9 pt-0">
                <h3 className="text-lg font-semibold text-primary mb-5">Online Profiles:</h3> {/* Added sub-header */}
                <ul className="space-y-9 mb-12"> {/* Increased spacing */}
                  {socialLinks.map((link, index) => (
                    <li key={index} className="flex items-center space-x-5 group/item">
                      <link.icon className="h-8 w-8 text-muted-foreground flex-shrink-0 transition-all duration-300 group-hover/item:scale-110 group-hover/item:text-accent group-hover/item:rotate-[6deg]" strokeWidth={1.5} /> {/* Smoother rotate */}
                      <Link
                        href={link.href}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-lg md:text-xl font-medium text-muted-foreground transition-colors duration-300 group-hover/item:text-accent group-hover/item:underline underline-offset-4 decoration-accent/70 flex items-center gap-1.5" // Added decoration color
                        aria-label={link.label}
                        data-cursor-interactive
                      >
                        {link.name}
                        <ArrowUpRight className="h-5 w-5 opacity-0 group-hover/item:opacity-100 transition-all duration-300 group-hover/item:translate-x-1 group-hover/item:-translate-y-1" /> {/* Enhanced arrow animation */}
                      </Link>
                    </li>
                  ))}
                </ul>
                <h3 className="text-lg font-semibold text-primary mb-5 pt-6 border-t">My Resume:</h3> {/* Added sub-header */}
      ...
      <style jsx>{`
        .pattern-circuit-board { /* Ensure this matches globals.css or is unique */
            ...
            background-size: 28px 28px; /* Slightly adjusted */
        }
        ...
        /* Additional blob animation styles for variety */
